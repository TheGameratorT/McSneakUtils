...
public class SeparatorListEntry extends AbstractConfigListEntry<Boolean> {
	...
	public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta) {
		...
		textRenderer.drawWithShadow(matrices, fieldName, x + (halfEntryWidth - halfTextWidth), (y + 6), 0xFFFFFF);
		int marginLinePad = 2;
		int textLinePad = 8;
		int firstLineEnd = (x + halfEntryWidth) - halfTextWidth;
		drawHorizontalLine(matrices, x + marginLinePad, firstLineEnd - textLinePad, y + 9, 0xFFFFFFFF);
		drawHorizontalLine(matrices, firstLineEnd + textWidth + textLinePad - 2, x + entryWidth - marginLinePad - 2, y + 9, 0xFFFFFFFF);
	}
